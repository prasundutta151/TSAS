...
int main(int argc, char *argv[]){
  ...
  long seed, NGrid;
  FILE *fp;
  char PSPFILE[128], INPFILE[128], OUTFITS[128];
  char param[128];

  if(argc!=6){
    printf("\nUSAGE : %s <PSPFILE> <OUTFITS> <seed> <BMAJ> <BMIN>\n", argv[0]);
    printf("\nPSPFILE: Power spectrum file <U (kl)>  <PS>\n");
    printf("OUTFITS: Output FITS file\n");
    printf("seed   : seed for the random number generator\n");
    printf("BMAJ   : Beam Major axis (deg)\n");
    printf("BMIN   : Beam Minor axis (deg)\n");
    printf("\n");
    return 1;
  }
  sscanf(argv[1], "%s", PSPFILE);
  sscanf(argv[2], "%s", OUTFITS);
  seed = (long)atoi(argv[3]);
  BMaj = (double)atof(argv[4]);
  BMin = (double)atof(argv[5]);
