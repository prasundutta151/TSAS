...
min_chi_index = np.where(Chiv == np.min(Chiv))[0]
print min_chi_index
tau0min = tau0[min_chi_index]
sigmmin = sigm[min_chi_index]
alphmin = alph[min_chi_index]
tau0_min = tau0min
tau0_min = 1.
...
plt.ylim(sigm2D[0]+dx/2., sigm2D[NG-1]+dy/2.)
